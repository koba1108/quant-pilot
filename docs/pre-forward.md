...
Replay validates the content-addressed artifact, loads the exact retained configuration, strategy, inputs, and Universe snapshot, rebuilds the decision from its recorded opening state, compares the canonical payload, and verifies the ledger named by that retained configuration. A later current-config ledger-path change cannot redirect historical replay. Replay does not apply another transition.
