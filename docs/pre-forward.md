...
Replay validates the content-addressed artifact, loads the exact retained configuration, strategy, inputs, and Universe snapshot, rebuilds the decision from its recorded opening state, compares the canonical payload, and verifies its ledger binding. It does not apply another transition.
