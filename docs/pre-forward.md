...
The SQLite ledger is append-only at the database level: update/delete triggers reject mutations, each transition is hash-chained, and the run key permits exactly one normal Decision Package per portfolio/Asia-Tokyo market calendar month. Historical verification checks the schema and exact append-only trigger definitions through a read-only, non-creating connection. On POSIX systems, the runtime directory and database are restricted to `0700` and `0600`.
