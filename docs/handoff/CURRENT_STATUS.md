...
- The maximum-three-holding constraint and -30% high-water-mark stop are asserted at the config, Decision Package, execution, and integration-test boundaries. Safety liquidation remains authoritative only after complete Corporate Action/distribution coverage through the current decision cutoff proves the stored unit basis; a gap after the latest price still blocks valuation and liquidation rather than turning a split into a false drawdown. Synthetic `pre-forward-daily-bars-v3` fixtures explicitly declare complete no-event coverage through the fixture cutoff, while credentialed retained bars do not.
...
- Each decision stores the exact validated pre-forward config, any nested credentialed-sample config, and the exact Universe master as immutable content-addressed artifacts. Replay loads those artifacts rather than later mutable files; regressions change execution/config versions and ledger paths, invalidate the original sample-config file, and append a valid future-dated master revision, then still reproduce the original Decision Package exactly.
...
- Active-branch verification: Bun 1.3.14 `bun test` 212 pass / 0 fail; Bun 1.2.14 per-file sweep 212 pass / 0 fail and Pre-Forward CLI pass with strict SQLite shutdown; `bunx tsc --noEmit` pass; fixture execute/repeat/replay across later config, ledger-path, nested sample-config, and Universe revisions, ledger-uncommitted orphan Decision handling, artifact-store relocation rejection, D-009 marginal/aggregate-cost rejection, Tokyo-market-month intramonth rejection, creation-time provenance, held-event-coverage/chronology guards, absolute-timestamp artifact validation, listing/intraday availability and loaded-input/config integrity validation, and retained live-audit blocking behave as expected; `git diff --check` passes.
...
- PASS — a credentialed decision remains replayable from its retained nested sample config after the original sample-config file is changed or unavailable
