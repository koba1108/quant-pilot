...
- The run key permits one normal cycle per portfolio/Asia-Tokyo market calendar month. Offset-formatted timestamps are normalized to the Tokyo market date before cycle assignment. Before opening the mutable current ledger path, ordinary execution indexes retained Decision artifacts for the same portfolio and follows their retained ledger binding, preventing path relocation from duplicating a monthly run or resetting a later cycle to initial cash. A future-cycle path change is rejected until an explicit audited ledger migration exists. A different cutoff in the same market month is rejected until a defined D-009/O-009 emergency mode and audit contract are approved.
...
- PASS — a current ledger-path revision cannot redirect historical replay, duplicate a retained monthly run, or reset a later cycle without an explicit audited migration
