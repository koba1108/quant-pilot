...
- The run key permits one normal cycle per portfolio/Asia-Tokyo market calendar month. Offset-formatted timestamps are normalized to the Tokyo market date before cycle assignment. `pre-forward-runtime-binding-v2` creates a per-portfolio owner-only enrollment directory before input loading and pins both physical artifact-root and ledger paths. A missing record inside an existing directory, an incomplete set, or a legacy v1 record requires explicit audited recovery. Ordinary execution starts from every committed ledger run for the configured portfolios, loads its exact Decision Package from the pinned store, and verifies both sides before portfolio advancement. A package left before a losing or interrupted append is immutable but ignored as uncommitted, while a missing committed package blocks. This binding and ledger-driven reconciliation prevent concurrent alternate first-cycle ledgers, simultaneous artifact/ledger relocation, duplicated monthly runs, and resets to initial cash. Artifact-root changes and future-cycle ledger-path changes are rejected until an explicit audited migration exists. A different cutoff in the same market month is rejected until a defined D-009/O-009 emergency mode and audit contract are approved.
