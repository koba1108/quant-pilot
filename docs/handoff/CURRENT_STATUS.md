...
- The run key permits one normal cycle per portfolio/Asia-Tokyo market calendar month. Offset-formatted timestamps are normalized to the Tokyo market date before cycle assignment. `pre-forward-runtime-binding-v3` validates or initializes the configured ledger before publishing a binding, then creates a per-portfolio owner-only enrollment directory before input loading and pins strategy identity plus both physical artifact-root and ledger paths. A malformed first-run ledger therefore leaves no portfolio binding and can be corrected without manual binding cleanup. A missing record inside an existing directory, an incomplete set, or an older binding schema requires explicit audited recovery. Ordinary execution starts from every committed ledger run for the configured portfolios, loads its exact Decision Package from the pinned store, and verifies both sides before portfolio advancement. A package left before a losing or interrupted append is immutable but ignored as uncommitted, while a missing committed package blocks. This binding and ledger-driven reconciliation prevent cross-strategy portfolio reassignment, concurrent alternate first-cycle ledgers, simultaneous artifact/ledger relocation, duplicated monthly runs, and resets to initial cash. Strategy reassignment, artifact-root changes, and future-cycle ledger-path changes are rejected until an explicit audited amendment or migration exists. A different cutoff in the same market month is rejected until a defined D-009/O-009 emergency mode and audit contract are approved.
...
- Active-branch verification: Bun 1.3.14 `bun test` 215 pass / 0 fail; Bun 1.2.14 per-file sweep 215 pass / 0 fail and Pre-Forward CLI pass with strict SQLite shutdown; `bunx tsc --noEmit` pass; fixture execute/repeat/replay across later config, ledger-path, nested sample-config, original M1 artifact-directory removal, and Universe revisions, complete credentialed-lineage retention, ledger-uncommitted orphan Decision handling, pre-decision artifact/ledger binding, missing-binding/ledger/committed-package blocking, artifact-store relocation rejection, D-009 marginal/aggregate-cost rejection, Tokyo-market-month intramonth rejection, creation-time provenance, held-event-coverage/Universe-eligibility/chronology guards, absolute-timestamp artifact validation, listing/intraday availability and loaded-input/config integrity validation, and retained live-audit blocking behave as expected; `git diff --check` passes.
