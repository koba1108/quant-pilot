...
- Bun SQLite stores an append-only run index and hash-chained state transitions. Update/delete triggers reject mutation; historical indexing/replay opens retained ledgers read-only without create permission and validates the schema plus exact guard definitions. A missing retained ledger blocks rather than creating a replacement or resetting to initial cash. Runtime directories and databases are owner-only on POSIX.
- The run key permits one normal cycle per portfolio/Asia-Tokyo market calendar month. Offset-formatted timestamps are normalized to the Tokyo market date before cycle assignment. A fixed owner-only runtime binding outside the configurable artifact store pins each portfolio to its physical artifact root. Before opening the mutable current ledger path, ordinary execution validates that binding and indexes only Decision artifacts whose exact ID is committed and verified in their retained ledger. A package left before a losing or interrupted append is immutable but ignored as uncommitted, so it cannot poison duplicate detection. The retained ledger binding prevents simultaneous artifact/ledger relocation from hiding history, duplicating a monthly run, or resetting a later cycle to initial cash. Artifact-root changes and future-cycle ledger-path changes are rejected until an explicit audited migration exists. A different cutoff in the same market month is rejected until a defined D-009/O-009 emergency mode and audit contract are approved.
...
- Active-branch verification: Bun 1.3.14 `bun test` 213 pass / 0 fail; Bun 1.2.14 per-file sweep 213 pass / 0 fail and Pre-Forward CLI pass with strict SQLite shutdown; `bunx tsc --noEmit` pass; fixture execute/repeat/replay across later config, ledger-path, nested sample-config, original M1 artifact-directory removal, and Universe revisions, complete credentialed-lineage retention, ledger-uncommitted orphan Decision handling, missing-ledger non-recreation, artifact-store relocation rejection, D-009 marginal/aggregate-cost rejection, Tokyo-market-month intramonth rejection, creation-time provenance, held-event-coverage/Universe-eligibility/chronology guards, absolute-timestamp artifact validation, listing/intraday availability and loaded-input/config integrity validation, and retained live-audit blocking behave as expected; `git diff --check` passes.
