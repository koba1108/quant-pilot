...
Implementation note (2026-08-29): `src/data/provider-evaluation.ts` and `research/provider-evaluation/o001-candidates.json` evaluate explicit source bundles without scoring or selecting a winner. An authorized local audit retained successful J-Quants responses for five JPX codes over 2026-04-20..22 and HTTP 404 responses for all five tested EODHD `.TSE` symbols. A bounded current-account discovery probe also found no Japan/XJPX/Tokyo exchange and no Japanese `search/1308` result. Both bundles remain `blocked`, `selection=not_selected`, and unable to enable `etf_realistic`; EODHD is retained as an overseas/FX candidate, not a JPX daily-price comparator. No production license or provider choice is approved. See `docs/provider-evaluation.md`.
