...
- active M2 branch: Bun 1.3.14 `bun test` 215 pass / 0 fail and Bun 1.2.14 per-file compatibility sweep 215 pass / 0 fail; TypeScript passes; synthetic Trend/Rotation execute through D-009 benefit gates; duplicate invocation and explicit replay are non-mutating even after later config, ledger-path, nested sample-config, original M1 artifact-directory removal, and Universe master revisions; the complete credentialed raw-response/daily-bar/observation/audit lineage is validated and pinned before historical use; v2 runtime bindings pin artifact and ledger paths before the first decision; an uncommitted valid Decision artifact is ignored only when its retained ledger proves another exact artifact was committed, while a missing binding, ledger, or committed Decision Package blocks without recreation or another transition; artifact-store relocation, Tokyo-market-month intramonth reruns, aggregate costs at or above 100%, and held valuation/liquidation after `lastEligibleDate` reject; strict SQLite shutdown, held-unit event coverage, chronology/absolute-timestamp/listing/intraday/input-integrity regressions pass; retained J-Quants input blocks with no cash movement
