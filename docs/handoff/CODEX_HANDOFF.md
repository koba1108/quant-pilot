...
4. makes duplicate invocation and explicit Decision Package replay from the exact retained pre-forward/sample configs, retained ledger path, inputs, and Universe non-mutating and deterministic, rejects a second normal cutoff in the same Asia-Tokyo market month, and separates actual package creation time from market `asOf`;
