...
- `src/pre-forward/runtime-binding.ts`: `pre-forward-runtime-binding-v3` owner-only binding directories plus an independent `.quant-pilot/` `pre-forward-runtime-enrollment-set-v1` manifest atomically published only after the complete binding set and ledger validate/initialize; interrupted pre-publication setup can resume only without committed runs, while published evidence fails closed when generated bindings disappear or conflict
...
- `bun test`: 223 pass / 0 fail
- Bun 1.2.14 direct aggregate `bun test`: 223 pass / 0 fail; all 25 files register through `bun:test`
- `bunx tsc --noEmit`: success
...
- test-runner regression: every test file registers with `bun:test`, so the required aggregate `bun test` gate executes the same 223 tests on Bun 1.2.14 and Bun 1.3.14 instead of silently skipping `node:test` registrations
