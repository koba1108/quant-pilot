...
- Bun 1.2.14 direct aggregate `bun test`: 224 pass / 0 fail; all 25 files register through `bun:test`
- `bunx tsc --noEmit`: success
...
- test-runner regression: every test file registers with `bun:test`, so the required aggregate `bun test` gate executes the same 224 tests on Bun 1.2.14 and Bun 1.3.14 instead of silently skipping `node:test` registrations
