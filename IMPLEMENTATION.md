...
- `src/pre-forward/runner.ts`: explicit-`asOf` execute/replay CLI, retained-artifact run-key lookup before ledger selection, one normal run per portfolio/Asia-Tokyo market month, actual package-creation provenance, and fail-closed credentialed-audit loading
...
- ledger replay regression: a later current-config ledger path cannot redirect replay or ordinary duplicate detection, and a future cycle rejects relocation until an explicit audited migration exists
